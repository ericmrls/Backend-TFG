...
En este caso se trata del modulo All, que utiliza el iterador MINDIteratorAll,
con toda la información disponible de la noticia.
...
class MindHelperAllIterator:
    ...
    modulo All, que utiliza el iterador All, con toda la información disponible
    de la noticia.
    ...
    def __init__(self,
        ...
        hparams = prepare_hparams(dataset.yaml_file.format(model), 
                          ...
                          body_size=50,
                          epochs=1, # Una sola epoca ya que está preentrenado
                          show_step=10,
        ...
        if model == 'naml':
            self.model = NAMLModel(hparams, iterator, seed=seed)
    ...
    def __get_user_data(self, index):
        ...
        click_ab_index = self.model.test_iterator.news_ab_index[self.model.test_iterator.histories[index]]
        click_vert_index = self.model.test_iterator.news_vert_index[self.model.test_iterator.histories[index]]
        click_subvert_index = self.model.test_iterator.news_subvert_index[self.model.test_iterator.histories[index]]
